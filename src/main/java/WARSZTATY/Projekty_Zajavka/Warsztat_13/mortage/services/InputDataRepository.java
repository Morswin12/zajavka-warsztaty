...
public class InputDataRepository {
    ...
    private static Map<String, List<String>> readFile() {
        try {
            ...
            log.error("Error loading data, interrupting", exception);
            return Map.of();
        }
    }
    ...
    public Optional<InputData> read(){
        ...
        return Optional.of(InputData.builder()
                ...
                .rateType(Optional.ofNullable(inputData.get("installmentType")).map(MortgageType::valueOf).orElseThrow())
                ...
                .mortgageRateNumberToPrint(Optional.ofNullable(inputData.get("mortgageInstallmentNumberToPrint")).map(Integer::parseInt).orElseThrow())
                .build());
    }
